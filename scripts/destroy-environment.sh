...
# Step 2: Run Terraform destroy (remove Kubernetes resources from state first to avoid provider issues)
echo ""
echo "=== Step 1: Removing Kubernetes resources from Terraform state ==="
cd $TERRAFORM_DIR
...
# Step 3: Clean up GuardDuty-managed resources if VPC still exists
if [ "$VPC_ID" != "None" ] && [ "$VPC_ID" != "null" ] && [ -n "$VPC_ID" ]; then
    echo ""
    echo "=== Step 3: Cleaning up AWS auto-provisioned resources in VPC ==="
